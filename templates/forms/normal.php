...
<form id="normalproduct" class="form-order needs-validation display-block" name="order_form" action="/order/order" method="get">
        ...
        <?php if($formDate == "US"){ ?>
        <input class="form-control" id="userDobUS" name="userDobUS" placeholder="MM/DD/YYYY" type="number" pattern="[0-9]*" inputmode="numeric" required value="<?php if(isset($_SESSION['dobUS']))echo $_SESSION['dobUS']; ?>"/>
        <span class="icon-inside"><i class="fa fa-clock"></i> </span>
        <label for="userDobUS">Date of Birth</label>
        <?php }else{ ?>
        <input class="form-control " id="userDob" name="userDob" placeholder="DD-MM-YYYY" type="number" pattern="[0-9]*" inputmode="numeric" required value="<?php if(isset($_SESSION['dob']))echo $_SESSION['dob']; ?>"/>
        <span class="icon-inside"><i class="fa fa-clock"></i> </span>
